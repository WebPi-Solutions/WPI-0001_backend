...
5. **CI de deploy.** Los workflows `.github/workflows/build-and-push-*.yml` deben ejecutar **unitarios y e2e** (`npm test` y `npm run test:e2e`) **antes** de construir y empujar la imagen a GHCR. Si un test falla, el job aborta y **no** se publica. No quites esos pasos ni los conviertas en `continue-on-error`. Stripe, Firebase, Dropbox, OpenAI y cualquier SDK de terceros se mockean: nunca claves ni APIs reales.
